	...
	__renderFooter: function (){
		...
		if(this.state.data&&this.state.data.length){
			if(this.state.current<this.state.total){
				return <div onClick={()=>this.loadingMore()} className="footer">点击加载更多 当前 ({this.state.current}/{this.state.total}) 页 {this.state.count} 条</div>;
			}else {
				return <div className="footer">共 ({this.state.current}/{this.state.total}) 页 {this.state.count} 条</div>;
			}
		}
	},
	render: function () {
		return (
			<DownPuller ref="owner" onDownPullEnd={this.__onDownPullEnd} >
				<div className={"zr-pull-refresh-list " + this.props.className }>
					{this.__render()}
					{this.__renderFooter()}
				</div>
			</DownPuller>
