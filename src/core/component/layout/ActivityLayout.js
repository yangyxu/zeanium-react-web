...
module.exports = React.createClass({
	...
	render: function(){
		...
		return (
			<div className={zn.react.classname("zr-basic-layout", "zr-activity-layout", "direction-" + this.props.direction, this.props.className)}>
				<div className="layout-header" style={_styles.header}>{_children[0]}</div>
				{!!this.props.barWidth && <div className="layout-body" style={_styles.body}>{this.__bodyRender()}</div>}
				<div className="layout-footer" style={_styles.footer}>{_children[1]}</div>
			</div>
		);
	}
});
