...
module.exports = React.createClass({
    ...
    drawSelectedPointsStatus: function (status){
        var _ctx = this._ctx,
            _radius = this._radius;
        this._selectedPoints.forEach(function (point) {
    ...
    drawSelectedPointsLines: function (point){
        var _ctx = this._ctx;
        _ctx.beginPath();
        _ctx.lineWidth = 3;
    ...
    __isMatchPoint: function (currPoint, point){
    ...
    __endHandler: function (event) {
        if(this._touching){
            this._touching = false;
            this.validate();
            setTimeout(()=>this.createPoints(), this.props.delay);
        }
    },
    validate: function (){
        var _value = this._selectedPoints.map(function (point, index){ return point.index; });
        var _obj = {
            boolValue: false
        };
        if(this.state.value){
