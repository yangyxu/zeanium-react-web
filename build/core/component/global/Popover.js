...
var Popover = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			'div',
			{ className: zn.react.classname('zr-popover zr-arrow zr-arrow-color-white', this.state.arrowClassName, this.props.className), style: zn.extend(this.props.style, _style) },
			this.props.closeable && React.createElement('i', { className: 'popover-close fa fa-close zr-hover-self-loading' }),
			this.props.content
		);
	}
});
...
module.exports = zn.popover = zn.Class({
	...
	methods: {
		...
		render: function render(content, options) {
			...
			return this._popover = ReactDOM.render(React.createElement(Popover, _extends({}, options, { content: content })), this._dom), this._popover;
		},
		close: function close(tag) {
			if (this._popover) {
