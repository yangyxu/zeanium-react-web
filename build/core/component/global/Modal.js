	...
	methods: {
		...
		middle: function middle(content, config) {
			...
			return this.open(React.createElement(
				'div',
				{ className: 'modal-middle', style: zn.extend({ visibility: 'hidden' }, _config.style) },
				content,
				_config.closeable && React.createElement('i', { onClick: this.close.bind(this), className: 'close fa fa-times zr-hover-self-loading' })
			), zn.extend({
				showOverlay: true,
		...
		full: function full(content, config) {
			return this.open(React.createElement(
				...
				content,
				React.createElement('i', { onClick: this.close.bind(this), className: 'close fa fa-times zr-hover-self-loading' })
			), config);
		},
