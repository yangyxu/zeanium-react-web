...
var Tooltip = React.createClass({
	...
	render: function render() {
		return React.createElement(
			'div',
			{ className: zn.react.classname("zr-tooltip zr-arrow zr-arrow-color-black zr-arrow-placement-center", this.props.className), style: this.props.style },
			this.props.content
		);
	}
});
...
zn.tooltip = zn.Class({
	...
	methods: {
		...
		render: function render(content, options) {
			this._tooltip = ReactDOM.render(React.createElement(Tooltip, _extends({}, options, { content: content })), this._dom);
		},
		close: function close() {
			if (this._tooltip) {
				this._tooltip.close();
