...
module.exports = React.createClass({
	...
	__onPreview: function __onPreview(item) {
		zn.modal.full(React.createElement(
			'div',
			{ style: { display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'scroll', position: 'absolute', width: '100%', height: '100%' } },
	...
	__getFileTypeRender: function __getFileTypeRender(value, type) {
		...
		return React.createElement(
			...
			React.createElement('i', { className: "icon fa fa-" + (type || 'file') }),
			React.createElement(
				'a',
				{ target: '_blank', href: zn.http.fixURL(value) + "?download=true", onClick: function onClick(event) {
						return event.stopPropagation();
					}, 'data-tooltip': _file, className: 'name' },
	...
	render: function render() {
		...
		return React.createElement(
			'div',
			{ className: zn.react.classname("zr-files", this.props.className), style: this.props.style },
			React.createElement(
				'ul',
				{ className: 'files' },
