...
var TreeColumn = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			...
			React.createElement(
				'div',
				{ className: 'inner-content', onClick: this.__onClick },
				React.createElement(
					'span',
					{ className: 'tabs', style: { width: this.props.depth * 16 } },
					!!_hasChildren && React.createElement('i', { onClick: this.__onIconClick, className: "icon fa " + this.__getIconClassName() })
				),
				this.props.parent.props.checkboxEnabled && React.createElement(Checkbox, { checked: this.props.checked, disabled: this.props.parent.props.disabled, onChange: function onChange(event, value) {
						return _this.__onCheckboxChange(value);
					} }),
				this.__renderContent()
			),
			!!_hasChildren && React.createElement(Tree, _extends({}, this.props.parent.props, this.props, { active: this.state.active, data: this.state.data }))
		);
	}
});
...
var Tree = React.createClass({
	...
	__itemRender: function __itemRender(item, index) {
		...
		if (!_content && item) {
			...
			_content = React.createElement(TreeColumn, { parent: this, key: index,
	...
	render: function render() {
		return React.createElement(RTList, _extends({}, this.props, { className: zn.react.classname("zr-tree", this.props.className), itemRender: this.__itemRender, onLoaded: this.__onLoaded }));
	}
});
